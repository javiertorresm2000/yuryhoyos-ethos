...
export class ReportsComponent implements OnInit {
    ...
    public companies : Array<IOption> = []
    ...
    constructor(private api : ApiService, private formBuilder: FormBuilder, private router: Router, private excelService: ExcelService) {
        ...
        */
        ...
        api.getCompanies(companies => {
            this.companies = companies.map(c => ({ value: c.id, label:c.name }))
        })
        ...
        this.form.controls.source_id.disable()
    ...
    enableCompany(id) {
        ...
        this.api.getExternalCategories(id, categories => {
            this.categories = categories.map(c => ({ value: c.id, label:c.name }))
            this.form.controls.category_id.enable()
        }) 

        this.api.getExternalSources(id, sources => {
            this.sources = sources.map(c => ({ value: c.id, label:c.name }))
            this.form.controls.source_id.enable()
        }) 
    }
