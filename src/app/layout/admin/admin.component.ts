...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  constructor(public menuItems: MenuItems, public router: Router, private api: ApiService) {
    this.animateSidebar = '';
    this.navType = 'st2';
    ...
    console.log('constructor-current:' + this.router.url)
  }

  public userFullName = ""
  public photo = ""
  public notificationBlink: boolean
  public notifications : Array<{ id:number; name: string; message: string; date:string; }>
  ...
  ngOnInit() {
    ...
    if (User.isNotOwner()) {
      ...
      const welcomeMessage = localStorage.getItem("welcome")

      if (logoFromStorage != null) {
        this.logo = logoFromStorage
      }
      ...
      this.api.getCompany(user.company_id, (company) => {
        if (company.logo != logoFromStorage) {
          this.logo = company.logo
          localStorage.setItem("logo", company.logo)
        }
        const name = "Bienvenido a " + company.name
    ...
    this.api.getNotifications(notifications => {
    ...
    this.adminMenuItems = this.menuItems.getAll()
  }

  redirect(id) {
      console.log(id)
      this.router.navigate(['/denouncement/' + id])
  }
  ...
  isActiveChildState(asideItem) {
    //console.log('current:' + this.router.url)
    
    if (this.router.url == ('/' + asideItem.state)) {
      
        return true
    }

    if (asideItem.other_state ==  (this.router.url.replace(/\/\d+/, ''))) {
        return true
    }

    if (asideItem.other_state) {
        console.log(this.router.url +':'+asideItem.other_state)
    }
    
    return this.router.url == asideItem.other_state
  }
